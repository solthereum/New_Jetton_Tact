...
function initJettonDefaultWallet_init_args(src: JettonDefaultWallet_init_args) {
    return (builder: Builder) => {
        ...
        b_0.storeAddress(src.owner);
        b_0.storeAddress(src.master);
        b_0.storeAddress(src.jetton_owner);
    };
}
...
async function JettonDefaultWallet_init(owner: Address, master: Address, jetton_owner: Address) {
    ...
    initJettonDefaultWallet_init_args({ $$type: 'JettonDefaultWallet_init_args', owner, master, jetton_owner })(builder);
...
export class JettonDefaultWallet implements Contract {
    ...
    static async init(owner: Address, master: Address, jetton_owner: Address) {
        return await JettonDefaultWallet_init(owner, master, jetton_owner);
    }
    
    static async fromInit(owner: Address, master: Address, jetton_owner: Address) {
        const init = await JettonDefaultWallet_init(owner, master, jetton_owner);
